...
export default function ChipLookupField({
    ...
    return (
        <div
            ...
            aria-invalid={error}
            className={`flex w-full items-center overflow-hidden rounded-md border transition-[border-color,box-shadow] duration-150 ${error ? 'border-red-500 focus-within:border-red-500 focus-within:shadow-input-error-focus' : 'border-slate-400 focus-within:border-[var(--color-input-focus)] focus-within:shadow-[0_0_0_3px_var(--color-input-focus-ring)]'} ${disabled ? 'bg-slate-100 text-slate-400 cursor-default' : error ? 'bg-red-500/5 cursor-text' : 'bg-white cursor-text'} ${heightClassName} ${className}`.trim()}
        >
