            ...
            <div className="flex min-w-0 flex-1 items-center gap-2 pl-1 pr-3">
                ...
                {hasSelectedValue ? null : (
                    <input
                        ...
                        autoCorrect="off"
                        spellCheck={false}
                        className={`h-full min-w-[2.5rem] flex-1 bg-transparent py-2 text-sm outline-none placeholder:text-disabled-border-t disabled:cursor-not-allowed disabled:text-slate-400 ${disabled ? 'text-slate-400' : 'text-slate-700'} indent-2 ${inputClassName}`.trim()}
                    />
                )}
            </div>
