...
export default function AccountLookupSearchInput({
    ...
    return (
        <div
            ...
            aria-invalid={error}
            className={`group flex w-full items-center overflow-hidden rounded-md border ${toneClassName} transition-[border-color,box-shadow] duration-150 ${disabled ? 'bg-slate-100 text-slate-400 cursor-default' : error ? `bg-red-500/5 ${wrapperCursor}` : `bg-white ${wrapperCursor}`} ${className}`.trim()}
        >
