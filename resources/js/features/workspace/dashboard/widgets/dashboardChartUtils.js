...
export function normalizeTrendSeries(series = [], accent = 'var(--color-blue-280)') {
    ...
    return (series ?? [])
        ...
        .map((item, index) => {
            const hasNegativeData = item.data.some((val) => Number(val) < 0);
            let color = item.color ?? item.borderColor ?? (index === 0 ? accent : 'var(--color-tab-view-active-text)');
            if (hasNegativeData && (!item.color || item.color === accent)) {
                color = '#f87171';
            }
            ...
            return {
                ...
                borderColor: color,
                backgroundColor: item.fillColor ?? toRgba(color, 0.16) ?? fallbackFill,
            };
        });
}
