                ...
                {items.map((item, index) => (
                    <div key={item.id ?? index} className="flex gap-6 py-4 border-b border-table-row-border last:border-b-0 first:pt-0">
                        <div className="flex flex-col items-center justify-center text-center min-w-[56px] shrink-0">
                            <span className="text-xs font-normal text-blue-7c839b">{item.dayName}</span>
                            <span className="text-4xl font-bold text-blue-434a65 leading-none my-1">{item.dayNum}</span>
                            <span className="text-xs font-normal text-blue-7c839b">{item.monthName}</span>
                        </div>

                        <div className="flex-1 flex flex-col justify-center gap-1.5">
                            <div className="flex items-center gap-2">
                                <span className="h-[10px] w-[10px] rounded-full border-2 border-blue-2563eb bg-white shrink-0" />
