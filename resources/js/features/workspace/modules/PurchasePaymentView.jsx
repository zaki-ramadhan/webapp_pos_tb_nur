...
import { buildPurchasePaymentConfig, buildPurchasePaymentRecord } from '@/features/workspace/modules/purchasePaymentConfig';
...
function PurchasePaymentFormView({ config, activeLevel2Tab }) {
    ...
    const dockActions = useMemo(
        () =>
            (values.dockActions ?? config.draft?.dockActions ?? [])
                ...
                .map((action) => (action.id === 'save' && !isDetail ? { ...action, tone: 'primary' } : action)),
        [config.draft?.dockActions, isDetail, values.dockActions],
    );
                ...
                header={
                    <div className="grid gap-x-8 gap-y-3 xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
                        ...
                        <div className="grid gap-y-3 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center sm:gap-x-4">
                            ...
                            <TransactionDateInput value={values.entryDate} className="max-w-[238px]" />
                        </div>
                    </div>
                }
...
export default function PurchasePaymentView({ page, mode, activeLevel2Tab, onOpenContent, onOpenDetail }) {
    ...
    return mode === 'table' ? (
        ...
        <PurchasePaymentFormView config={config} activeLevel2Tab={activeLevel2Tab} />
    );
}
