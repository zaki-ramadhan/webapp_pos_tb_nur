...
const stockOpeningColumns = [
    { id: 'date', label: 'Tanggal', widthClassName: 'w-[20%]', align: 'left' },
    { id: 'quantity', label: 'Kuantitas', widthClassName: 'w-[18%]', align: 'right' },
    { id: 'unit', label: 'Satuan', widthClassName: 'w-[16%]', align: 'left' },
    { id: 'unitCost', label: 'Biaya Satuan', widthClassName: 'w-[20%]', align: 'right' },
    { id: 'warehouse', label: 'Gudang', widthClassName: 'w-[26%]', align: 'left' },
];
...
function buildFallbackDetailRecord(row, config) {
    ...
    return {
        ...
        notes: row.notes ?? '',
        length: row.length !== null && row.length !== undefined && row.length !== '' ? formatAmountInput(row.length) : '',
        width: row.width !== null && row.width !== undefined && row.width !== '' ? formatAmountInput(row.width) : '',
        height: row.height !== null && row.height !== undefined && row.height !== '' ? formatAmountInput(row.height) : '',
        weight: row.weight !== null && row.weight !== undefined && row.weight !== '' ? formatAmountInput(row.weight, { allowDecimal: false }) : '',
