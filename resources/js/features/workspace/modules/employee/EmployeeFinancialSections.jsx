...
export function EmployeeTaxTab({ form, values, onChange }) {
    return (
        <div className="max-w-[1000px] space-y-3">
            ...
            <EmployeeFieldRow label="No. NPWP"><TextInput name="tax_number" value={values.taxNumber} onChange={(event) => onChange('taxNumber', event.target.value)} className="h-[40px] rounded-[4px] border-[#cfd6e2] md:max-w-[476px]" inputClassName="text-xs sm:text-sm text-[#1f2436]" /></EmployeeFieldRow>
            <EmployeeFieldRow label="Status Pekerja">
                <SelectField name="employment_status" value={values.employmentStatus} onChange={(event) => onChange('employmentStatus', event.target.value)} containerClassName="w-full md:max-w-[676px]" className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-xs sm:text-sm text-[#1f2436]">
                    {form.employmentStatusOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                </SelectField>
            </EmployeeFieldRow>
            <EmployeeFieldRow label="Dikenakan PTKP"><TextInput name="tax_allowance_applies" value={values.taxAllowanceApplies} readOnly className="h-[40px] rounded-[4px] border-[#cfd6e2] md:max-w-[176px]" inputClassName="text-xs sm:text-sm text-[#5f6780]" /></EmployeeFieldRow>
            <EmployeeFieldRow label="Status PTKP">
                <SelectField name="tax_allowance_status" value={values.taxAllowanceStatus} onChange={(event) => onChange('taxAllowanceStatus', event.target.value)} containerClassName="w-full md:max-w-[676px]" className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-xs sm:text-sm text-[#1f2436]">
            ...
            <EmployeeFieldRow label="PPh mulai dihitung">
                <div className="flex flex-wrap items-center gap-4">
                    <SelectField name="tax_start_month" value={values.taxStartMonth} onChange={(event) => onChange('taxStartMonth', event.target.value)} containerClassName="w-full md:max-w-[376px]" className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-xs sm:text-sm text-[#1f2436]">
                        {form.taxStartMonthOptions.map((option) => <option key={option} value={option}>{option}</option>)}
                    </SelectField>
                    <SelectField name="tax_start_year" value={values.taxStartYear} onChange={(event) => onChange('taxStartYear', event.target.value)} containerClassName="w-full md:max-w-[176px]" className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-xs sm:text-sm text-[#1f2436]">
            ...
            <EmployeeFieldRow label="Penghasilan Sebelumnya">
                <TextInput name="previous_income" value={values.previousIncome} onChange={(event) => onChange('previousIncome', event.target.value)} className="h-[40px] rounded-[4px] border-[#cfd6e2] md:max-w-[376px]" inputClassName="text-xs sm:text-sm text-[#1f2436]" />
            </EmployeeFieldRow>
            <EmployeeFieldRow label="PPh Sebelumnya">
                <TextInput name="previous_tax" value={values.previousTax} onChange={(event) => onChange('previousTax', event.target.value)} className="h-[40px] rounded-[4px] border-[#cfd6e2] md:max-w-[376px]" inputClassName="text-xs sm:text-sm text-[#1f2436]" />
            </EmployeeFieldRow>
            <div className="ml-0 max-w-[860px] border-l-[4px] border-[#8c8c8d] pl-3 text-sm italic leading-6 text-[#ff4a4a] lg:ml-[220px]">{form.taxCalculationNote}</div>
        </div>
    );
}
