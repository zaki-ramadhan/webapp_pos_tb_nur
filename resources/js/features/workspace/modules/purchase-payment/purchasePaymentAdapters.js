    ...
    return applyPurchasePaymentInvoices(
        {
            ...
            autoNumber: false,
            numberingType: record.numbering_type ?? config.numberingOptions?.[0] ?? 'Nomor Bukti Kas (Tunai)',
...
export function buildFormState(source = {}, config) {
    return applyPurchasePaymentInvoices({
        ...
        numberingType: (config.numberingOptions?.includes(source.numberingType) ? source.numberingType : (config.numberingOptions?.includes(config.draft?.numberingType) ? config.draft?.numberingType : config.numberingOptions?.[0])) ?? 'Nomor Bukti Kas (Tunai)',
