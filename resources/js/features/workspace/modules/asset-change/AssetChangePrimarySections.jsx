            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,520px)] xl:items-start">
                <div className="space-y-3">
                    <AssetChangeFieldRow label={config.labels.changeType}>
                        <SelectField value={values.changeType} disabled={isDetail} onChange={(event) => setValues((current) => ({ ...current, changeType: event.target.value }))} containerClassName="w-full sm:max-w-[334px]" className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-[15px] text-[#1f2436]">
                    ...
                    <AssetChangeFieldRow label={config.labels.lastDepreciation}>
                        <TextInput value={values.lastDepreciation} readOnly className="h-[40px] rounded-[4px] border-[#cfd6e2] sm:max-w-[334px]" inputClassName="text-[15px] text-[#6b7280]" />
                    </AssetChangeFieldRow>
                </div>
                ...
                <div className="space-y-3 xl:justify-self-end xl:min-w-[420px]">
                    {isDetail ? (
                        <AssetChangeFieldRow label={config.labels.number} required labelClassName="sm:text-right">
                            <TextInput value={values.documentNumber} readOnly trailing={<span className="text-[22px] font-semibold text-[#1f2436]">x</span>} className="h-[40px] rounded-[4px] border-[#cfd6e2]" inputClassName="text-[15px] text-[#1f2436]" trailingClassName="px-3" />
                        </AssetChangeFieldRow>
                    ) : (
                        <div className="grid gap-3 sm:grid-cols-[200px_minmax(0,1fr)] sm:items-center sm:gap-x-4">
                            ...
                            {values.autoNumber ? (
                                ...
                                <TextInput value={values.documentNumber} onChange={(event) => setValues((current) => ({ ...current, documentNumber: event.target.value }))} className="h-[40px] rounded-[4px] border-[#cfd6e2]" inputClassName="text-[15px] text-[#1f2436]" />
                            )}
                        </div>
                    )}
                    ...
                    <AssetChangeFieldRow label={config.labels.bookValue} labelClassName="sm:text-right">
                        <TextInput value={values.bookValue} readOnly className="h-[40px] rounded-[4px] border-[#cfd6e2] sm:max-w-[230px]" inputClassName="text-right text-[15px] text-[#6b7280]" />
                    </AssetChangeFieldRow>
                </div>
            </div>
        </div>
            ...
            <div className="mt-4 grid gap-5 lg:max-w-[980px] lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
                <div className="space-y-4">
                    <AssetChangeFieldRow label={config.labels.depreciationMethod}>
                        <SelectField value={values.depreciationMethod} disabled={isDetail} onChange={(event) => setValues((current) => ({ ...current, depreciationMethod: event.target.value }))} className="h-[40px] rounded-[4px] border-[#cfd6e2]" selectClassName="text-[15px] text-[#1f2436]">
                    ...
                    <AssetChangeFieldRow label={config.labels.residualValue}>
                        <FormattedAmountInput value={values.residualValue} readOnly={isDetail} onChange={(event) => setValues((current) => ({ ...current, residualValue: event.target.value }))} prefix="Rp" className="h-[40px] rounded-[4px] border-[#cfd6e2] sm:max-w-[406px]" inputClassName="text-right text-[15px] text-[#1f2436]" prefixClassName="min-w-[46px] justify-center px-0 text-[#7a8498]" />
                    </AssetChangeFieldRow>
                </div>
                <div className="space-y-4">
                    <AssetChangeFieldRow label={config.labels.changeNotes} alignTop>
                        <TextareaField value={values.changeNotes} readOnly={isDetail} onChange={(event) => setValues((current) => ({ ...current, changeNotes: event.target.value }))} rows={4} className="rounded-[4px] border-[#cfd6e2]" textareaClassName="min-h-[140px] text-[15px] text-[#1f2436]" />
                    </AssetChangeFieldRow>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="min-h-[560px]">
            <div className="flex flex-col gap-3 border-b border-[#d8dde7] pb-3 lg:flex-row lg:items-start lg:justify-between">
                <AccountLookupTextInput value={values.expenseSearch} placeholder={config.expenseSearchPlaceholder} className="h-[40px] rounded-[4px] border-[#cfd6e2] lg:max-w-[560px]" inputClassName="text-[15px] text-[#1f2436]" dialogTitle="Pilih Akun Pengeluaran" searchLabel="Cari akun pengeluaran" onSelectAccount={(_, label) => setValues((current) => ({ ...current, expenseSearch: label }))} />
                <div className="text-right text-[22px] font-normal text-[#1f2436]">{config.labels.expenseTitle}</div>
            </div>
