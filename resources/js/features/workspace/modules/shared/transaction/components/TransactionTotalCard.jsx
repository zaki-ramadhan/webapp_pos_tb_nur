...
export function TransactionTotalCard({ label, value, className = '' }) {
    ...
    return (
        ...
        >
            <div className="px-4 py-2.5 text-xs sm:text-sm text-brand-dark truncate">{label}</div>
            <div className="px-4 pb-3 text-right text-base font-semibold text-text-darkest whitespace-nowrap tabular-nums">{formattedValue}</div>
        </div>
    );
}
