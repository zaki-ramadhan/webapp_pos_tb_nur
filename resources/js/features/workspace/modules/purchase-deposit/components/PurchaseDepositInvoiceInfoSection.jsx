...
export default function PurchaseDepositInvoiceInfoSection({ config, values }) {
    return (
        <div className="w-full max-w-[540px]">
            <TransactionSectionHeading title={config.summaryTitle || 'Informasi Faktur'} icon="payment" />
                ...
                <div className="py-1">
                    {(values.summary || []).map(([label, value]) =>
                        label === 'Status' ? (
                            <div
                                key={label}
                                className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-ui-border-lightest px-4 py-2"
                            >
                            ...
                            <div
                                key={label}
                                className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b border-ui-border-lightest px-4 py-2 last:border-b-0"
                            >
                                <span className="text-xs sm:text-sm text-brand-dark font-normal">{label}</span>
