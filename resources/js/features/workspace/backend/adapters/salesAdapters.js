...
export function mapProductRow(record) {
    ...
    return {
        ...
        categoryName: record.category?.name ?? 'Umum',
        unit: record.base_unit?.name ?? record.base_unit?.code ?? record.baseUnit?.name ?? record.baseUnit?.code ?? record.unit ?? 'Pcs',
        unitName: record.base_unit?.name ?? record.base_unit?.code ?? record.baseUnit?.name ?? record.baseUnit?.code ?? record.unitName ?? record.unit ?? 'Pcs',
