    ...
    return (
        <div className={`relative ${resolvedContainerClassName}`.trim()}>
            ...
            <div
                ...
                className={`group flex ${heightClass} w-full items-center overflow-hidden rounded-md border transition-[border-color,box-shadow] duration-150 ${toneClassName} ${disabled ? 'bg-ui-bg-panel' : resolvedError ? 'bg-red-500/5' : 'bg-white'} ${cleanedClassName}`.trim()}
            >
                <button
                    ref={triggerRef}
                    id={id}
