...
export function DataTable({ className = '', wrapperClassName = '', bordered = true, children, ...props }) {
    ...
    const isTableBordered = bordered === 'table';
    const borderClass = (bordered === true) && !isExplicitlyBorderFree ? 'border border-table-border' : '';
    const tableBorderClass = bordered === 'x'
        ? 'border-x border-table-border'
        : isTableBordered
...
export function DataTableHead({ className = '', children, style: propStyle, onResizeStart = null, ...props }) {
    ...
    return (
        <th
            className={`border-r border-table-header-cell-border ${pxClass} py-2 ${alignClass} text-xs sm:text-sm font-normal leading-5 last:border-r-0 whitespace-nowrap truncate relative select-none ${cleanedClassName}`.trim()}
