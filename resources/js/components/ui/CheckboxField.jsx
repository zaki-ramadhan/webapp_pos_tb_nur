        ...
        <div className={`${widthClass} ${containerClassName}`.trim()}>
            <div className={`flex ${alignClassName} gap-2.5 text-xs sm:text-sm leading-6`.trim()}>
                <input
                    ...
                    className={`${inputOffsetClassName} shrink-0 rounded-[4px] border border-slate-400 text-input-brand focus:ring-2 focus:ring-input-focus/30 disabled:border-gray-200 disabled:bg-[#f5f5f5] disabled:text-gray-400 ${sizeClassName} ${resolvedError ? 'border-error-border' : ''} ${inputClassName}`.trim()}
