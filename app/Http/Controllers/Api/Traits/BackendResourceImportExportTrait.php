...
trait BackendResourceImportExportTrait
{
    public function import(Request $request, string $resource): JsonResponse
    {
        ...
        try {
            DB::transaction(function () use ($blueprint, $rows, $relationClassMap, $attributeMap, $messages, &$created, &$errors) {
                foreach ($rows as $index => $row) {
                  ...
                  // Cek kelengkapan kolom impor
                    ...
                    $validator = Validator::make($cleanRow, $rules, $messages, $customAttributes);
                    if ($validator->fails()) {
                        $rowNum = $index + 2;
