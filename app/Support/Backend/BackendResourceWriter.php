...
class BackendResourceWriter
{
    ...
    protected function persist(BackendResourceBlueprint $blueprint, Model $record, array $payload): Model
    {
        return DB::transaction(function () use ($blueprint, $record, $payload): Model {
            ...
            if (!$record->exists) {
                ...
                } elseif ($blueprint->key === 'products') {
                    $code = $payload['code'] ?? '';
                    if (empty($code) || $this->isAutoGeneratedNumber($code, 'products')) {
                        $payload['code'] = $this->generateNextSequentialNumber('products');
                    }
                }
            }
    ...
    public function generateNextSequentialNumber(string $resourceKey, ?string $entryDate = null): string
    {
        ...
        $targetTable = $resourceKey === 'products' ? 'products' : 'operation_documents';
        $targetColumn = $resourceKey === 'products' ? 'code' : 'document_number';
